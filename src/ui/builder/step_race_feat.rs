...
pub fn render(app: &mut App, frame: &mut Frame) {
    ...
    let title = Paragraph::new(" Character Builder: Human – Versatile (Origin Feat) ")
        .style(
            Style::default()
                .fg(Color::Cyan)
    ...
    let list = List::new(items)
        .block(
            Block::default()
                .borders(Borders::ALL)
                .title(" Choose Origin Feat (no prerequisites) "),
        )
        .highlight_style(
            Style::default()
                .fg(Color::Yellow)
    ...
    let selected_name = filtered
        ...
        .map(|f| f.name.as_str())
        .unwrap_or("none");
    let chosen_text = if app.builder.bonus_feat_id.is_some() {
        format!(
            "Chosen: {}  |  Enter confirm  Tab skip  Esc back to skill",
            selected_name
        )
    } else {
        "↑↓ select  Enter confirm  Tab skip (no Versatile feat)  Esc back".to_string()
    };
    let help = Paragraph::new(chosen_text).style(Style::default().fg(Color::DarkGray));
...
pub fn handle_key(app: &mut App, key: KeyEvent) {
    ...
    match key.code {
        ...
        KeyCode::Enter => {
            // Confirm selection - filter same as render
            let feat_id = {
            ...
            app.builder.bonus_feat_id = feat_id;
            app.builder.feat_picker_search.clear();
            app.builder.feat_picker_index = 0;
            // Proceed to Class step
            app.builder.step = CharacterCreationStep::Class;
            app.builder.list_state.select(Some(0));
            app.status_msg = "Origin feat selected.".to_string();
        }
        _ => {}
    }
}
