...
pub fn render(app: &mut App, frame: &mut Frame, area: Rect) {
    ...
    if let Some(ref actions) = app.char_actions {
        if app.actions_sub_tab == ActionsSubTab::LimitedUse {
            let mut limited = actions.limited_use.clone();
            // Merge derived limited-use actions (like LOH/CD) if not present
            for la in local_actions {
                if la.max_uses.is_some() && !limited.iter().any(|a| a.name == la.name) {
                    limited.push(la);
                }
            }
...
fn render_action_list(app: &mut App, actions: &[ActionEntry], frame: &mut Frame, area: Rect) {
    if actions.is_empty() {
    ...
    let selected_idx = app.actions_list_state.selected().unwrap_or(0);
    ...
    let items: Vec<ListItem> = actions
        ...
        .map(|(i, action)| {
            ...
            if let Some(r) = &action.range { props.push(format!("Range: {}", r)); }
            if let Some(h) = &action.hit_bonus { props.push(format!("Hit/DC: {}", h)); }
            if let Some(d) = &action.damage { props.push(format!("Damage: {}", d)); }
            ...
            if let Some(desc) = &action.description {
                if !desc.is_empty() && desc != "null" {
                    ...
                    let cleaned = crate::ui::sheet::features::strip_tags(&desc_clean);
                    let snippet: String = cleaned.chars().take(100).collect();
                    let snippet = if cleaned.len() > 100 { format!("{}...", snippet) } else { snippet };
                    lines.push(Line::from(Span::styled(format!("    {}", snippet), Style::default().fg(Color::Gray))));
                }
            }

            if is_selected {
                lines.push(Line::from(Span::styled("    Press Shift+K for full details", Style::default().fg(Color::DarkGray))));
            }
